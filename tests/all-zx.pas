...
overlay procedure Test(S: String);
begin
  WriteLn;
  WriteLn('--- ', S, ' ', Copy('--------------------------', Length(S), 255));
  WriteLn;
...
begin
  ...
  while X <= Y do
  begin
    WriteLn('|', X:9:6, '|', Sin(X):9:6, '|', Cos(X):9:6, '|');
    X := X + Z;
    I := I + 1;
  end;
...
begin
  ...
  WriteLn('***************************');
  WriteLn('* Passed assertions: ', AssertPassed:4, ' *');
  WriteLn('* Failed assertions: ', AssertFailed:4, ' *');
  WriteLn('***************************');
  WriteLn;
end.