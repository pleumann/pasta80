...
var
  ...
  (* Lives here rather than in the platform units because every target needs
     it now, including the ones without file support: Read from the keyboard
     reports a bad number through it as well. Aliased onto a fixed label so
     the conversion routines in system.asm can reach it. *)
  LastError: Byte absolute '__lasterror';

(**
 * Queries the last I/O error and resets it to zero, so that further
 * operations can be made. Everything in the Block/Text layer starts with
 * "if LastError <> 0 then Exit", so a pending error suppresses subsequent
 * I/O until it has been read here -- the same model Turbo Pascal uses.
 *)
...
(**
 * Checks the last I/O error and terminates the program if there is one.
 * Calls to this are generated by the compiler in {$i+} mode; under {$i-}
 * the program is expected to ask IOResult itself.
 *)
procedure BDosThrow;
begin
  if LastError <> 0 then
  begin
    WriteLn('I/O error ', LastError);
    Halt;
  end;
end;
