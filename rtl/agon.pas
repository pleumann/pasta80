...
function MOSAPIOSCLI(I: Integer): Byte; register; external 'mos_call_oscli';

(**
 * Turbo Pascal 4.0+ Exec command. We take the strings, add a terminator
 * and skip the length when passing the address.
 *)
...
begin
  if Params = '' then
    ...
    Temp := Command + ' ' + Params + #00;

  DosExitCode := MOSAPIOSCLI(Addr(Temp)+1); //skip the length parameter
end;

(**
 * Checks the last BDOS (MOS) error and terminates the program if it is <> 0.
...
//  WriteLn('Closing handle #',R.C);
  ...
  if R.C <> 0 then //so we don't close all files. Each file needs closing individually.
  begin
